  ...
  return (
    <div className="financial-report" style={{ position: 'relative' }}>
            ...
            <thead>
              <tr>
                {firstRowAttributes.map(attribute => (
                  <th key={attribute} className="attribute-name" onClick={() => handleAttributeClick(attribute.toLowerCase().replace(' ', '_'))}>{attribute}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      {showChart && selectedAttribute && (
        <>
          <div className="chart-container-wrapper" style={{ position: 'absolute', top: `${chartTop}px`, left: `${chartLeft}px`, right: `${chartRight}px`, zIndex: 1, backgroundColor: backgroundColor, padding: '10px', paddingLeft: `${chartPaddingLeft}px` }}>
            ...
            <div className="chart-container" style={{ width: chartWidth, height: `${chartContainerHeight}px` }}>
              <ResponsiveContainer>
                <LineChart data={getChartData()} margin={{ top: 5, right: 30, left: 50, bottom: 5 }}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="value" stroke="#8884d8" strokeWidth={2} activeDot={{ r: 0 }} dot={{ r: 0 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
